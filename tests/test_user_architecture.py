...
class UserArchitectureContractTest(unittest.TestCase):
    ...
    def test_algorithm_robot_and_config_contract(self):
        ...
        self.assertIn("#define MOTOR_LONGITUDINAL_SPEED_LIMIT_RPM 520U", config)
        self.assertIn("#define MOTION_LONGITUDINAL_CRUISE_RPM    640.0f", config)
        self.assertIn("#define MOTION_CRUISE_RPM          480.0f", config)
        self.assertIn("#define MOTION_DIAGONAL_CRUISE_RPM 300.0f", config)
