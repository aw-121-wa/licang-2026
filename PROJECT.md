...
- 2026-09-06 减速入口过渡试验：保留用户当前 80 RPM/s 减速度，新增 `MOTION_DECEL_ENTRY_MS=300` 名义过渡时间。定距平移在已应用峰值速度与剩余距离减速包络之间使用二次圆滑连接，提前减速且不超过原制动包络；峰值包含轮速限幅影响，短距离缩小过渡宽度，20 RPM 以下沿用原末段。此为减速入口平滑，不是全程 jerk 受限轨迹；短距离加速结束、航向修正及紧急停车不受此平滑保证。实车扭动及距离仍待验证。

- 2026-09-06 减速切入左右扭动试验：平移航向修正增大、减小和换向统一经过 `MOTION_HEADING_SLEW_RPM_PER_S`（当前 120 RPM/s）限制，不再先跳到零或直接释放修正。低速相对限幅仍可立即收紧，STOP 和零速度广播硬停不变；实际扭动改善待实车验证。此条取代下文历史记录中的 30 RPM/s 描述。
