...
- `RoundPillar_OrbitAndGrab()` 先发送第一个红球请求，再以反转后的单一方向（`forward=-63 RPM`、`omega=+49 RPM`）在 20 ms 控制循环中非阻塞轮询 `MaixCamLink_TakeReply()`；从当前航向基准连续绕行至 `+450°` 后直接结束，不再执行反向 90°。识别成功立即停车稳定，执行 Group4，完成后保持当前 ContinuousYaw 继续绕桩并发送下一次请求；单向 450°使用现有 15000 ms 绕桩保护时间。
