...
- `RoundPillar_OrbitAndGrab()` 先发送第一个红球请求，再在顺时针 360°（`forward=+63 RPM`、`omega=-49 RPM`）和同轨迹反向 90°（两个量同时反号）的 20 ms 控制循环中非阻塞轮询 `MaixCamLink_TakeReply()`。识别成功立即停车稳定，执行 Group4，完成后保持当前 ContinuousYaw 继续绕桩并发送下一次请求。
